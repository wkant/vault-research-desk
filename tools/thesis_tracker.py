...
LOG_PATH = os.path.join(SCRIPT_DIR, "thesis_log.json")
...
def load_log():
    """Load the thesis log from disk. Returns a list of thesis dicts."""
...
def save_log(entries):
    """Persist the thesis log to disk."""
    with open(LOG_PATH, "w") as f:
        json.dump(entries, f, indent=2)


def find_active(entries, ticker):
    """Return the first ACTIVE thesis for a ticker, or None."""
    ticker = ticker.upper()
    for entry in entries:
        if entry["ticker"] == ticker and entry["status"] == "ACTIVE":
            return entry
    return None
...
def fmt_thesis(entry, verbose=True):
    ...
    if verbose:
        lines.append("    Thesis: {}".format(entry["thesis"]))
...
def cmd_add(args):
    ...
    existing = find_active(entries, ticker)
    if existing:
    ...
    entry = {
        ...
        "date_opened": today(),
        "key_conditions": args.conditions or "",
        "status": "ACTIVE",
        "history": [{"date": today(), "note": "Thesis opened."}],
    }

    if entry["direction"] not in VALID_DIRECTIONS:
        print("ERROR: direction must be one of {}".format(VALID_DIRECTIONS))
        return
    if entry["conviction"] not in VALID_CONVICTIONS:
...
def cmd_update(args):
    ...
    entries = load_log()
    ticker = args.ticker.upper()
    entry = find_active(entries, ticker)
    if not entry:
    ...
    note = args.note
    if args.conviction:
        old_conv = entry["conviction"]
        entry["conviction"] = args.conviction.upper()
        note += " [conviction {} -> {}]".format(old_conv, entry["conviction"])
    if args.direction:
    ...
    entry["history"].append({"date": today(), "note": note})
    save_log(entries)
...
def cmd_close(args):
    ...
    note = "Thesis closed: {} -> {}.".format(old_status, new_status)
    if args.note:
        note += " " + args.note
    entry["history"].append({"date": today(), "note": note})
...
def parse_report_table(text):
    ...
    for i, line in enumerate(lines):
        ...
        if "ticker" in lower_cells and any(
            a in lower_cells for a in ("action", "direction", "call")
        ):
        ...
        # Data row
        if header_idx is not None and col_map and len(cells) >= len(col_map):
            row = {}
        ...
        if existing:
            ...
            entry = {
                ...
                "history": [
                    {
                        "date": report_date,
                        "note": "Auto-extracted from report.",
                    }
                ],
            }
            entries.append(entry)
    ...
    for ticker, reason in avoid_list:
        ...
        if existing:
            ...
            existing["history"].append({
                "date": report_date,
                "note": "Closed by AVOID in report: {}".format(reason),
            })
